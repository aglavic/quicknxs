'''
Copy of the genx data module used to unpickle and pickle the
GenX template files.
'''
...
class DataSet:
    ...
    def __init__(self, name='', copy_from=None):
        ...
        if copy_from:
            ...
            #The columns to load
            self.cols=(0, 1, 1, 1) # Columns to load (xcol,ycol)
            #The different colors for the data and simulation
    ...
    def safe_copy(self, new_set):
        ...
        if new_set.xerror_raw is None:
          self.xerror=self.error
          self.xerror_raw=self.error_raw
        else:
          self.xerror=new_set.xerror
          self.xerror_raw=new_set.xerror_raw
        ...
        self.xerror_command=new_set.xerror_command
        try:
        ...
        # mds data loader addition, origin file location is saved
        if hasattr(new_set, 'origin'):
          self.origin=new_set.origin
    ...
    def save_file(self, filename):
        ...
        if  self.x.shape==self.y_sim.shape and \
            ...
            self.x.shape==self.xerror.shape and\
    ...
    def try_commands(self, command_dict):
        ...
        if (xt.shape!=yt.shape or xt.shape!=et.shape or xt.shape!=xet.shape or extra_shape)\
            and result=='':
            result+='The resulting arrays are not of the same size:\n'+\
                       'len(x) = %d, len(y) = %d, len(e) = %d, len(xe) = %d'\
                    %(xt.shape[0], yt.shape[0], et.shape[0], xet.shape[0])
            for key in self.extra_commands:
    ...
    def get_commands(self):
        ...
        '''
        cmds={'x':self.x_command, 'y':self.y_command,
              'e':self.error_command, 'xe':self.xerror_command}
        for key in self.extra_commands:
