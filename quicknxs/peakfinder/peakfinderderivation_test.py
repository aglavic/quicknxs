...
class TestPeakFinderDerivation(unittest.TestCase):
    ...
    def test_loadcsvfile(self):
        '''checking that loadCsvFile works correctly'''
        [xdata, ydata, edata] = loadCsvFile('peakfinder/easy_data_set.csv')
        xdata10 = xdata[0:10]
    ...
    def test_case_medium_data_set(self):
        '''using run 124211'''
        [xdata, ydata, edata] = loadCsvFile('peakfinder/medium_data_set.csv')
        peakfinder1 = PeakFinderDerivation(xdata, ydata, edata)
        peaks = peakfinder1.getPeaks()
        self.assertEqual(peaks, [151, 159])
    ...
    def test_case_hard_data_set(self):
        '''using run 124135'''
        [xdata, ydata, edata] = loadCsvFile('peakfinder/hard_data_set.csv')
        peakfinder1 = PeakFinderDerivation(xdata, ydata, edata)
        peaks = peakfinder1.getPeaks()
